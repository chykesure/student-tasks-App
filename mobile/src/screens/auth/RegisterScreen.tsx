...
const RegisterScreen: React.FC<RegisterScreenProps> = ({ navigation }) => {
  ...
  const handleRegister = async (): Promise<void> => {
    ...
    try {
      ...
      // Success: navigation usually handled in AuthContext via listener/useEffect
      // You can also manually navigate here if needed:
      // navigation.replace('Main'); // or whatever your app root is
      ...
      if (error.message?.includes('Network') || error.code === 'ECONNABORTED') {
        errorMessage = 'Network error: Cannot reach the server. Please check your internet connection or server status.';
      } else if (error.response) {
        // Server responded with error (e.g. 400, 409, 500)
        errorMessage = error.response.data?.message ||
          `Server error (${error.response.status}): ${error.message}`;
      } else if (error.request) {
        // Request sent but no response received → classic network failure
